...
export const inputValidationMiddleware = (req: Request, res: Response, next: NextFunction) => {
    let errors = validationResult(req).formatWith(customFormatter)

    // check if names of fields were written right
    const fieldNameError = detectErrorsInFieldNames(req.body)
...
function detectErrorsInFieldNames(data: Object) {
    ...
    if (extraFields.length > 0) {
        extraFields.forEach(field => {
            errors.push({ message: 'Unexpected field', field: `${missingFields[0]}` })
        });
    }
    return errors;
}
